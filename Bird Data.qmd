...
```{r}
ggplot(averages, aes(x = SiteID, y = Countln, fill = ifelse(grepl("^BSB", SiteID), "BSB", "CDL"))) +
...
## Barplot of Diversity by Site ID

```{r}
#Graph by total in each survey point
ggplot() +
  geom_bar(data = shannon_index, aes(x = SiteID, y = Shannon_Index, fill = ifelse(grepl("^BSB", SiteID), "BSB", "CDL")), stat = "identity", position = "dodge", color = "black") +
...
ggplot(shannon_index, aes(x=Date, y=Shannon_Index, group=SiteID, color=SiteID)) + 
  geom_line()

ggplot(shannon_index, aes(x = Date, y = Shannon_Index, fill = SiteID)) +
